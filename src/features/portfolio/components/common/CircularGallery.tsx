...
function deriveFontFamilyFromUrl(url: string): string {
  const fileName: any = (url.split("/").pop() || "custom-font").split("?")[0];
  const base: any = fileName.replace(/\.(woff2?|ttf|otf|eot)$/i, "");
...
async function loadFontFromStylesheet(url: string): Promise<string> {
  ...
  for (const block of faceBlocks) {
    const familyMatch: any = block.match(/font-family:\s*['"]?([^;'"]+)['"]?/);
    const urlMatch = block.match(/url\(\s*['"]?([^'")]+)['"]?\s*\)/);
    if (!familyMatch || !urlMatch) continue;
    family = familyMatch[1].trim();
    const descriptors: FontFaceDescriptors = {};
    const weightMatch: any = block.match(/font-weight:\s*([^;]+);/);
    const styleMatch: any = block.match(/font-style:\s*([^;]+);/);
    const rangeMatch: any = block.match(/unicode-range:\s*([^;]+);/);
    if (weightMatch) descriptors.weight = weightMatch[1].trim();
    if (styleMatch) descriptors.style = styleMatch[1].trim();
    if (rangeMatch) descriptors.unicodeRange = rangeMatch[1].trim();
    fontFaces.push(
      new FontFace(family as any, `url(${urlMatch[1]})`, descriptors),
    );
  }
...
async function resolveFont(font: string, fontUrl?: string): Promise<string> {
  ...
  try {
    ...
    const sizeMatch: any = font.match(/^\s*(.*?\d+px)/);
...
function getFontSize(font: string): number {
  const match: any = font.match(/(\d+)px/);
...
class App {
  ...
  onTouchDown(e: MouseEvent | TouchEvent | any) {
    this.isDown = true;
    this.scroll.position = this.scroll.current;
    this.start = "touches" in e ? e.touches[0].clientX : e.clientX;
  }

  onTouchMove(e: MouseEvent | TouchEvent | any) {
